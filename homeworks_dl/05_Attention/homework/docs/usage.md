...
Обучение поддерживает автоматическое сохранение и восстановление состояния:

**Автоматическое сохранение:**
- Чекпоинты сохраняются в `checkpoints/` каждую эпоху
- Лучшая модель сохраняется отдельно как `best_model.pth`
- История обучения и состояние оптимизатора сохраняются
...
# Пользовательская директория для чекпоинтов
python -m src.train --checkpoint-dir my_checkpoints

# Early stopping при отсутствии улучшений
python -m src.train --early-stopping 5
```

**Важно:** Директория `checkpoints/` игнорируется DVC и Git, поэтому чекпоинты сохраняются между запусками `dvc repro`.
